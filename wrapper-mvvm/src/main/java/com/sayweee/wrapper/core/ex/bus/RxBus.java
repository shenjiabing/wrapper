...
public class RxBus {
     ...
     * @param o
     */
    public void post(int code, IEvent o) {
        _bus.onNext(new BusEvent(code, o));
    }
    ...
    public <T> Observable<T> toObservable(final int code, final Class<T> eventType) {
        return _bus.ofType(BusEvent.class)
                .filter(new Predicate<BusEvent>() {
                    @Override
                    public boolean test(BusEvent event) throws Exception {
                        //过滤code和eventType都相同的事件
                        return event.code == code && eventType.isInstance(event.event);
                    }
                }).map(new Function<BusEvent, Object>() {
                    @Override
                    public Object apply(BusEvent event) throws Exception {
                        return event.event;
                    }
                }).cast(eventType);
    }
